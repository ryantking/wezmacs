...
- **Dependency metadata:** fields remain informational; there is no framework-wide availability check. The Git module now resolves its launchers explicitly and makes Broot/Jujutsu bindings opt-in; other modules still need their own audit.
- **Git/worktree navigation:** native comparisons and read-only Git worktree discovery live in the Git module. This does not reintroduce the removed agent orchestration or worktree lifecycle manager. See [Git module scope and environment boundaries](git.md).
