...
- **Plugin adapters:** `mux` loads third-party plugins during module evaluation, contains a Homebrew-specific zoxide path, duplicates status callbacks, and assumes theme colors are present when those callbacks run. Isolate/configure these adapters in a subsequent module pass rather than rebuilding the loader.
