...
return {
	...
	keys = function(opts)
		return {
			{ key = "Enter", mods = "SHIFT", action = act.SendString("\x1b\r"), desc = "newline" },
			LEADER = {
				a = {
					{ key = "a", action = wezmacs.action.SmartSplit(opts.agent), desc = "split" },
					{ key = "A", action = wezmacs.action.NewTab(opts.agent), desc = "tab" },
					{ key = "c", action = wezmacs.action.SmartSplit(opts.alt_agent), desc = "alt/split" },
					{ key = "C", action = wezmacs.action.NewTab(opts.alt_agent), desc = "alt/tab" },
